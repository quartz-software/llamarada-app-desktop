  ...
  function getRoomsData() {
    fetch("/api/rooms")
      ...
      .then((data: Room[]) => {
        setRoomsData(
          data.filter(
            (room: Room) => !prd.rooms.some((rr) => rr.id === room.id)
          )
        );
      })
      .catch((error) => {
    ...
    <div>
      ...
      >
        ...
        <div className="div--rooms">
          {searchRooms(roomFilter1, promotionData.rooms).map(
            (room: Room, index) => {
              return (
                <div key={index} className="room--item">
                  ...
                  <Button
                    handleClick={() => {
                      setPromotionData({
                        ...promotionData,
                        rooms: promotionData.rooms.filter(
                          (r: Room) => r.id !== room.id
                        ),
                      });
              ...
              <div key={index} className="room--item">
                ...
                <Button
                  handleClick={() => {
                    setPromotionData({
                      ...promotionData,
                      rooms: [...promotionData.rooms, room],
                    });
        ...
        <Button handleClick={postData}>Guardar</Button>
        <Button handleClick={() => nav("/promotions")}>Cancelar</Button>
      </form>
    </div>
  );
};
